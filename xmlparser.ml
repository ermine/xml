...
type uchar = ucs4
type ustring = uchar list

exception InvalidInput
exception LexerError of ustring
exception UnknownToken of ustring
exception UnknownEntity of ustring
exception InvalidChar of uchar
exception Finished
...
let get_word nextf state ucs4 =
  let is_ascii ucs4 =
    if ucs4 < 0xFF then
      ...
      false
  in
  let rec aux_get_word acc state ucs4 =
    if is_ascii ucs4 then
      next_char (aux_get_word (ucs4 :: acc))
    else
      let word = List.rev acc in
...
let parse_charref nextf state ucs4 =
  let parse_decimal_charref nextf state ucs4 =
    let rec get_decimal acc _state ucs4 =
      if ucs4 >= '0' && ucs4 <= '9' then
        next_char (get_decimal (acc * 10 + (ucs4 - '0')))
      else if ucs4 = ';' then
        nextf acc
      else
  ...
  parse_name
    (fun name state ucs4 ->
       if ucs4 = ';' then (
         (match name with
(*              
            | "lt" -> fencoder state buf; '<'
            | "gt" -> fencoder state buf '>'
            | "apos" -> fencoder state buf '\''
            | "quot" -> fencoder state buf '"'
            | "amp" -> fencoder state buf '&'
*)                
            | other ->
...
let parse_PEreference nextf state ucs4 =
  parse_name (fun name _state ucs4 -> if ucs4 = ';' then
                nextf name
              else
...
let parse_contentspec nextf state ucs4 =
  let rec get_list nextf acc _state ucs4 =
    if ucs4 = '|' then
      ...
      if List.length acc = 1 then
        ...
        next_char  (expected_char '*' (next_char (nextf acc)))
    else
      raise (LexerError "expected '|' or ')'")
  in
  ...
  in
    get_word
      (fun word state ucs4 ->
         if word = "EMPTY" then
           skip_blank (expected_char '>' (nextf `EMPTY)
                      ) state (UCS4 ucs4)
         else if word = "ANY" then
           skip_blank (expected_char '>' (nextf `ANY)
                      ) state (UCS4 ucs4)
         else if word = "" then
           get_mixed (fun mixed state ucs4 ->
                        skip_blank
                          (expected_char '>' (nextf mixed)
                          ) state (UCS4 ucs4)
                     ) state ucs4
         else
    ...
    after_blank
      (get_word
         (fun word state ucs4 ->
            match word with
              | "CDATA" ->
                  nextf name `CDATA state ucs4
              | "ID" ->
                  nextf name `ID state ucs4
              | "IDREF" ->
                  nextf name `IDREF state ucs4
              | "IDREFS" ->
                  nextf name `IDREFS state ucs4
              | "ENTITY" ->
                  nextf name `ENTITY state ucs4
              | "ENTITIES" ->
                  nextf name `ENTITIES state ucs4
              | "NMTOKEN" ->
                  nextf name `NMTOKEN state ucs4
              | "NMTOKENS" ->
                  nextf name `NMTOKENS state ucs4
              | "NOTATION" ->
...
let parse_entitydecl nextf state ucs4 =
  let get_end ent state ucs4 =
    skip_blank (expected_char '>' (nextf (`EntityDecl ent)))
      state (UCS4 ucs4)
  in
...
let parse_eq nextf state ucs4 =
  skip_blank (expected_char '=' (Lexer (skip_blank nextf))
             ) state (UCS4 ucs4)
...
let rec parse_attributes tag attrs nextf state ucs4 =
  if ucs4 = '>' then
    ...
    next_char (expected_char '>' (nextf tag attrs false))
  else
    let smth state ucs4 =
      if ucs4 = '>' then
        nextf tag attrs true
...
let parse_end_element nextf state ucs4 =
  parse_name
    (fun name state ucs4 ->
       skip_blank (expected_char '>' (nextf name)) state (UCS4 ucs4)
    ) state ucs4
